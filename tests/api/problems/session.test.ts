...
describe("SolveSession flow", () => {
  ...
  it("TODO->IN_PROGRESS->TRIED->IN_PROGRESS->SOLVED", async () => {
    ...
    let sessions = await getSolveSessionsFromDB(testUser0.id, testProblem0.id);
    ...
    sessions = await getSolveSessionsFromDB(testUser0.id, testProblem0.id);
    expect(sessions.length).toBe(1);
    expect(sessions[0].finishedAt).toBeNull();
    expect(sessions[0].startedAt).toBeInstanceOf(Date);
    expect(await getProblemStatus(testUser0.id, user0Problem0Id)).toBe(
      "IN_PROGRESS",
    );
    ...
    expect(sessions.map((s) => s.id)).toEqual(
      sessionsBeforeNoteUpdate.map((s) => s.id),
    );
    expect(sessions.map((s) => s.duration)).toEqual(
      sessionsBeforeNoteUpdate.map((s) => s.duration),
    );
  });
  ...
  it("sessions are isolated between problems", async () => {
    ...
    const problem0SessionsBefore = await getSolveSessionsFromDB(
      testUser0.id,
      testProblem0.id,
    );
    ...
    let problem1Sessions = await getSolveSessionsFromDB(
      testUser0.id,
      testProblem1.id,
    );
    ...
    const problem0SessionsAfter = await getSolveSessionsFromDB(
      testUser0.id,
      testProblem0.id,
    );
    expect(problem0SessionsAfter.length).toBe(2);
    expect(problem0SessionsAfter.map((s) => s.id)).toEqual(
      problem0SessionsBefore.map((s) => s.id),
    );

    // IN_PROGRESS -> SOLVED on problem1: session is finished
    await finishProblem(user0Problem1Id, "SOLVED");
    problem1Sessions = await getSolveSessionsFromDB(
      testUser0.id,
      testProblem1.id,
    );
    expect(problem1Sessions.length).toBe(1);
    expect(problem1Sessions[0].finishedAt).toBeInstanceOf(Date);
    expect(await getProblemStatus(testUser0.id, user0Problem1Id)).toBe(
      "SOLVED",
    );
  });
  ...
  it("sessions are isolated between users", async () => {
    ...
    await startProblem(user0Problem0Id);
    expect(
      (await getSolveSessionsFromDB(testUser0.id, testProblem0.id)).length,
    ).toBe(1);
    expect(
      (await getSolveSessionsFromDB(testUser1.id, testProblem0.id)).length,
    ).toBe(0);
    ...
    const user1SessionsAfterUser0Solve = await getSolveSessionsFromDB(
      testUser1.id,
      testProblem0.id,
    );
    ...
    const user0SessionsAfterUser1Tried = await getSolveSessionsFromDB(
      testUser0.id,
      testProblem0.id,
    );
    ...
    const user1Sessions = await getSolveSessionsFromDB(
      testUser1.id,
      testProblem0.id,
    );
    ...
    const user1FinalSessions = await getSolveSessionsFromDB(
      testUser1.id,
      testProblem0.id,
    );
    expect(user1FinalSessions.length).toBe(2);
    expect(user1FinalSessions.every((s) => s.finishedAt !== null)).toBe(true);
    expect(await getProblemStatus(testUser1.id, user1Problem0Id)).toBe(
      "SOLVED",
    );
